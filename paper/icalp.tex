...
unusable as a program. Nothing is stored, so evaluating an entry of $M^{k}$
re-evaluates $M$ along every walk of length $k$ afresh, and no intermediate
result is ever shared; the cost grows exponentially in $k$, and even a small
election does not finish.
...
reused. Exponentiation is done by repeated squaring on binary numerals, so
$M^{k}$ costs a logarithmic number of matrix products rather than $k$ of
them. The two representations are then proved to agree:
\[
