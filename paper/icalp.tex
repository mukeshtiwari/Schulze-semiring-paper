...
Interpreting social choice axioms over a bounded semiring turns a flat list
of guarantees into a hierarchy, and the hierarchy is exact at the top. The
two structural guarantees (that the outcome is an ordering, and that a
winner exists) are each \emph{equivalent} to selectivity with the meet
property, hence to the carrier being $\max$ and $\min$ on a total order.
Schulze's arithmetic is a consequence of the guarantees, not a modelling
choice. Alongside the algebra, several criteria carry profile conditions
that no carrier can discharge. Independence of clones cannot be stated over a
fixed finite type of alternatives, but parameterising the closure by a list
removes that obstruction, and the criterion then joins the hierarchy at the
same level and turns out to be equivalent to winner existence rather than
merely to need the same algebra. The same closure also gives Smith-IIA in its
genuine removal form, on both sides of the cut: deleting the non-Smith
alternatives from the ballot, one at a time or all at once, and equally
deleting a Smith alternative, rather than neutralising them in the matrix.
The two operations agree because an alternative with no links is exactly one
that a closure over a sublist can ignore. It also settles the size of the
clone characterisation: five alternatives are optimal, because over the
diamond the clone criterion holds outright at four. One question remains
open: whether width is the exact boundary for counterexamples at three
alternatives.
