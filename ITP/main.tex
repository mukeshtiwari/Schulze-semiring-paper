...
Since an earlier version of this paper, the development has grown to
verify the social choice properties of the Schulze method over the
semiring abstraction itself: neutrality, the Pareto criterion,
monotonicity at the winner level, Condorcet consistency, the Smith
criterion, Smith-IIA on both sides of the cut (Schulze's 4.7.5a and
4.7.6), reversal symmetry, independence of clones, prudence, and the
MinMax set, together with characterisation theorems showing that
transitivity, winner existence, and independence of clones each hold for
every profile exactly when the carrier is selective and multiplication is
a meet, that is, exactly over the bottleneck semirings, and that the
five-alternative bound of the clone characterisation is optimal.
...
$a^{*} = \overline{1}$; and (iii) extend to $\omega$-continuous
semirings, which would enable verified Newton's method for algebraic
program analysis~\cite{esparza2010newtonian}.
